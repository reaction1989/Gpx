...
namespace Gps
{
    struct GpsLocation
    {
        ...
        public string ToString(string format)
        {
            ...
            {
                if (!int.TryParse(format.Substring(1), out var precision)) throw new FormatException();
        ...
        private static bool TryParseGpsValue(string s, ref int pos, out double result)
        {
            ...
            if (!TryParseValue(s, ref pos, out var degrees)) return false;
            ...
            if (!TryParseValue(s, ref pos, out var minutes))
            {
                result = degrees;
                return true;
            }
            ...
            if (!TryParseValue(s, ref pos, out var seconds))
            {
